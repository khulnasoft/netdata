...
Are you just getting started with Netdata Cloud? You can find simplified instructions in the [Install Netdata documentation](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/README.md#get-started)
...
By connecting a node, you opt-in to sending data from your Agent to Netdata Cloud via the [ACLK](https://github.com/khulnasoft/netdata/blob/master/aclk/README.md). This
...
Nodes** in the [Spaces management area](https://github.com/khulnasoft/netdata/blob/master/docs/cloud/manage/organize-your-infrastrucutre-invite-your-team.md#netdata-cloud-spaces#manage-spaces).
...
- when you are on the [Nodes tab](https://github.com/khulnasoft/netdata/blob/master/docs/cloud/visualize/nodes.md) and want to connect a node - this process falls into the [Manage Space](#manage-space-or-war-room) flow
...
When coming from the [Nodes tab](https://github.com/khulnasoft/netdata/blob/master/docs/cloud/visualize/nodes.md) the room parameter is already defined to current War Room.

### Connect an agent running in Linux

If you want to connect a node that is running on a Linux environment, the script that will be provided to you by Netdata Cloud is the [kickstart](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/README.md#automatic-one-line-installation-script) which will install the Netdata Agent on your node, if it isn't already installed, and connect the node to Netdata Cloud. It should be similar to:
...
For more details on what are the extra parameters `claim-token`, `claim-rooms` and `claim-url` please refer to [Connect node to Netdata Cloud during installation](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/methods/kickstart.md#connect-node-to-netdata-cloud-during-installation).
...
restarts using a persistent volume.  See our [recommended `docker run` and Docker Compose examples](https://github.com/khulnasoft/netdata/blob/master/packaging/docker/README.md#create-a-new-netdata-agent-container) for details.
...
To connect a node that is running on a macOS environment the script that will be provided to you by Netdata Cloud is the [kickstart](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/methods/macos.md#install-netdata-with-our-automatic-one-line-installation-script) which will install the Netdata Agent on your node, if it isn't already installed, and connect the node to Netdata Cloud. It should be similar to:
...
Read our [Kubernetes installation](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/methods/kubernetes.md#connect-your-kubernetes-cluster-to-netdata-cloud)
...
You can now move on to connecting. When you connect with the [kickstart](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/README.md#automatic-one-line-installation-script) script, add the `--claim-proxy=` parameter and
...
If you're having trouble connecting a node, this may be because the [ACLK](https://github.com/khulnasoft/netdata/blob/master/aclk/README.md) cannot connect to Cloud.
...
script](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/README.md#install-on-linux-with-one-line-installer).
...
We recommend you reinstall Netdata with a [static build](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/methods/kickstart.md#static-builds), which uses an
...
To fix this issue, reinstall Netdata using your [preferred method](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/README.md) and do not add the
...
GitHub](https://github.com/khulnasoft/netdata/issues/new?assignees=&labels=bug%2Cneeds+triage&template=BUG_REPORT.yml&title=The+installer+failed+to+prepare+the+required+dependencies+for+Netdata+Cloud+functionality)
...
GitHub](https://github.com/khulnasoft/netdata/issues/new?assignees=&labels=bug%2Cneeds+triage&template=BUG_REPORT.yml&title=ACLK-available-is-false)
with details about your system and relevant output from `error.log`.
...
using the [ACLK](https://github.com/khulnasoft/netdata/blob/master/aclk/README.md).
...
| enabled        | yes                       | The runtime option to disable the [Agent-Cloud link](https://github.com/khulnasoft/netdata/blob/master/aclk/README.md) and prevent your Agent from connecting to Netdata Cloud.         |
...
Our recommendation is to trigger the connection process using the [kickstart](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/README.md#automatic-one-line-installation-script) whenever possible.
