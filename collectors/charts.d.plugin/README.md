...
To better understand the guidelines and the API behind our External plugins, please have a look at the [Introduction to External plugins](https://github.com/khulnasoft/netdata/blob/master/collectors/plugins.d/README.md) prior to reading this page.
...
By default, `charts.d.plugin` is not included as part of the install when using [our official native DEB/RPM packages](https://github.com/khulnasoft/netdata/blob/master/packaging/installer/methods/packages.md). You can install it by installing the `netdata-plugin-chartsd` package.

## Configuration

`charts.d.plugin` itself can be [configured](https://github.com/khulnasoft/netdata/blob/master/docs/configure/nodes.md#use-edit-config-to-edit-configuration-files) using the configuration file `/etc/netdata/charts.d.conf`. This file is also a BASH script.
